      ...
      <main className="mx-auto flex max-w-md flex-col px-6 pt-8">
        <Card size="lg" className="flex flex-col">
          ...
          <h1 className="mt-1 font-serif text-[28px] font-semibold tracking-[-0.01em] text-ink">
            {players[0]?.nickname !== undefined ? `${players[0].nickname}'s room` : 'The room'}
          </h1>
          <p className="mt-1 font-sans text-[14px] text-ink-3">Waiting for the host to pick a game.</p>

          <div className="mt-5 flex flex-col gap-2">
            {players.map((p, i) => (
              <PlayerPill key={p.id} name={p.nickname} seat={seatForIndex(i)} size="sm" isYou={p.id === myId} />
            ))}
            {players.length === 0 ? (
              <p className="py-4 text-center font-sans text-[13px] text-ink-3">
                {status === ConnectionStatus.RECONNECTING ? 'Reconnecting…' : 'Joining…'}
              </p>
            ) : null}
          </div>
        ...
        <button type="button" onClick={leave} className="mt-4 self-center font-sans text-[13px] font-bold uppercase tracking-[0.1em] text-ink-3 hover:text-ink">
          Leave room
        </button>
      </main>
    </div>
