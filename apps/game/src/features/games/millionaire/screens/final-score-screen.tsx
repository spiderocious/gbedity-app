...
// FINAL SCORE — end-of-game celebration. Confetti, an emoji that adapts to how far up the ladder
// the player climbed, the total banked in ₦, and Play again / Home actions.
// Pure UI: caller passes totals + handlers.
...
// Emoji + headline based on how high the player climbed (share of questions correct).
        ...
        <div ref={stack} className="flex w-full max-w-xl flex-col items-center gap-5">
          ...
          <div data-stagger className="flex flex-col items-center gap-1">
            <span className="font-sans text-[12px] font-extrabold uppercase tracking-[0.18em] text-surface/90">
              Who Wants to Be a Millionaire · {eliminated ? 'Eliminated' : 'Complete'}
            </span>
          ...
          {/* Total banked — solid white on green (Score ink tone is invisible on green) */}
          <div data-stagger className="flex flex-col items-center">
            <span className="font-sans text-[13px] font-extrabold uppercase tracking-[0.14em] text-surface/80">
              Total banked
            </span>
            <span className="font-serif text-[64px] font-semibold leading-none tabular-nums text-surface sm:text-[80px]">
              {formatRung(totalBanked)}
            </span>
          </div>
