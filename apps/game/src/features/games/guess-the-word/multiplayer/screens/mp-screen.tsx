...
import {
  MpAudience,
  useMpGuessTheWord,
} from "../logic/use-mp-guess-the-word.ts";
...
export function MpGuessTheWordScreen({
  audience,
  code,
}: MpGuessTheWordScreenProps) {
  ...
  useEffect(() => {
    ...
    navigate(
      pathWith(isHost ? ROUTES.HOST_RESULT : ROUTES.PLAYER_RESULT, { code }),
    );
  ...
  switch (view.phase) {
    case GtwPhase.TURN_INTRO: {
      const guesserName =
        view.guesserId !== null ? nameOf(view.guesserId) : "Someone";
      ...
      return withHostControls(
        <SlideFrame
          tone={SlideTone.STAGE}
          compact
          animateKey={`intro-${view.turnIdx}`}
        >
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex flex-col gap-1">
                  {upNext.map((id, i) => (
                    <span
                      key={id}
                      className="font-sans text-[14px] text-surface/80"
                    >
    ...
    case GtwPhase.GUESSING: {
      const amGuesser =
        view.isGuesser || (isSpectator && view.guesserId === null);
    ...
    case GtwPhase.REVEAL: {
      ...
      return withHostControls(
        <SlideFrame tone={tone} compact animateKey={`reveal-${view.turnIdx}`}>
          <div className="flex flex-col items-center gap-5 text-center">
            <span
              className={`font-sans text-[12px] font-extrabold uppercase tracking-[0.18em] ${correct ? "text-surface/70" : "text-ink-3"}`}
            >
              Round {view.turnIdx + 1} · Reveal
            </span>
            <p
              className={`font-serif text-[36px] font-semibold leading-tight ${correct ? "text-surface" : "text-ink"}`}
            >
              {isMyReveal
                ? correct
                  ? "You got it!"
                ...
                : correct
                  ? `${guesserName} got it!`
                  : `${guesserName} missed it`}
            </p>
            {view.word !== null ? (
              <div
                className={`flex flex-col items-center gap-1 ${correct ? "text-surface/80" : "text-ink-3"}`}
              >
            ...
            {view.guessText !== null ? (
              <div
                className={`flex flex-col items-center gap-1 ${correct ? "text-surface/70" : "text-ink-3"}`}
              >
                <span className="font-sans text-[11px] font-extrabold uppercase tracking-[0.14em]">
              ...
              <p
                className={`font-sans text-[14px] ${correct ? "text-surface/70" : "text-ink-3"}`}
              >
                {isMyReveal
                  ? "Time ran out"
                  : `${guesserName} didn't guess in time`}
              </p>
            )}
            <p
              className={`font-sans text-[13px] ${correct ? "text-surface/60" : "text-ink-4"}`}
            >
    ...
    default: {
      const myRow =
        myId !== undefined
          ? view.board.find((r) => r.playerId === myId)
          : undefined;
      const totalScore = isSpectator
          ...
          <div className="flex flex-col items-center gap-6 text-center">
            ...
            {isSpectator && topName !== null ? (
              ...
              <>
                ...
                <p className="font-sans text-[15px] text-surface/80">
                  Your final score
                </p>
              </>
            )}
            {view.board.length > 0 ? (
              <div className="flex flex-col gap-2 w-full max-w-xs">
                {view.board.slice(0, 5).map((row, i) => (
                  <div
                    key={row.playerId}
                    className="flex items-center justify-between gap-2 rounded-xl bg-surface/10 px-4 py-2"
                  >
                    <span className="font-sans text-[14px] text-surface/80">
                      {i + 1}.{" "}
                      {row.playerId === myId ? "You" : nameOf(row.playerId)}
                    </span>
                    <span className="font-sans text-[14px] font-bold text-surface">
                      {row.points} pts
                    </span>
                  </div>
                ))}
            ...
            <button
              ...
              onClick={() =>
                navigate(
                  pathWith(isHost ? ROUTES.HOST_LOBBY : ROUTES.PLAYER_LOBBY, {
                    code,
                  }),
                )
              }
...
function GuesserPanel({
  ...
  const timerColor =
    pct > 0.5 ? "text-surface" : pct > 0.25 ? "text-warn" : "text-warn-deep";

  return (
    <SlideFrame tone={SlideTone.ACTION} compact animateKey={animateKey}>
      <div className="flex flex-col items-center gap-6 text-center">
        <span className="font-sans text-[12px] font-extrabold uppercase tracking-[0.18em] text-surface/70">
          Your turn to guess
        </span>
        ...
        <div
          className={`font-mono text-[52px] font-black leading-none ${timerColor}`}
        >
          {Math.ceil(secondsLeft)}
          <span className="font-sans text-[16px] font-normal text-surface/60">
            s
          </span>
        </div>
        ...
        {view.wordLength !== null ? (
          <div className="flex flex-col items-center gap-2">
            <span className="font-sans text-[13px] text-surface/70">
              The word has
            </span>
            <span className="font-serif text-[36px] font-bold text-surface">
              {view.wordLength}
            </span>
            <span className="font-sans text-[13px] text-surface/70">
              letters
        ...
        {view.word !== null ? (
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="font-sans text-[28px] font-bold text-surface">
              {view.word}
            </span>
          </div>
        ) : null}
...
function AudiencePanel({
  ...
  const masked = '•'.repeat(word.replace(/\?/g, '').length);
  const hideTimer = useRef<number | null>(null);

  const scheduleHide = (): void => {
    if (hideTimer.current !== null) window.clearTimeout(hideTimer.current);
    hideTimer.current = window.setTimeout(() => setRevealed(false), 2000);
  };
      ...
      <div className="flex flex-col items-center gap-6 text-center">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          {!revealed ? (
            <button
              type="button"
              onClick={() => { setRevealed(true); scheduleHide(); }}
              className="mt-1 font-sans text-[12px] font-bold uppercase tracking-[0.14em] text-surface/50 underline underline-offset-2 hover:text-surface/80"
            >
              Tap to reveal
            </button>
          ) : null}
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <span className="font-mono text-[28px] font-bold text-surface">
        ...
        <div className="flex flex-col items-center gap-3">
          <span className="font-sans text-[13px] font-bold text-surface/80">
            Questions remaining:{" "}
            <span className="text-surface">{questionCount}</span>
          </span>
          ...
          {isModerator ? (
            ...
            <p className="font-sans text-[13px] text-surface/60">
