...
// §7.1 — player result. LIVE by default: the real final board from the game-over snapshot + a
// 15s auto-return to the lobby (GameResult). `?mock` opts into the static preview summary
// (/preview-screens). Fixes the "fake 3rd / 940 pts" mock leaking onto a real game.
export function PlayerResultScreen() {
  const [search] = useSearchParams();
  const { code = MOCK_ROOM_CODE } = useParams();

  if (search.get('mock') === null) {
    return <GameResult code={code} lobbyRoute={ROUTES.PLAYER_LOBBY} />;
  }
  return <MockPlayerResult code={code} />;
}

// Static preview (the /preview-screens gallery) — illustrative numbers only.
function MockPlayerResult({ code }: { readonly code: string }) {
